...
class ScriptHandler {
    ...
    public static function copyCeviFiles(Event $event) {
        ...
        if (!$fs->exists($drupalRoot . '/modules/custom')) {
            $fs->mkdir($drupalRoot . '/modules/custom');
        }

        if (!$fs->exists($drupalRoot . '/themes/custom')) {
            $fs->mkdir($drupalRoot . '/themes/custom');
        }

        $fs->mirror($drupalRoot . '/../../cevi-modules', $drupalRoot . '/modules/custom');
        $fs->mirror($drupalRoot . '/../../cevi-themes', $drupalRoot . '/themes/custom');

        // Copy Settings-Files
        if (!$fs->exists($drupalRoot . '/../../settings/settings.current.php')) {
